...
const App: React.FC = () => {
  ...
  const hasPermission = (permission: string) => {
    ...
    const permissions = (PERMISSIONS as any)[currentUser.role] || [];
...
const BookOutModal: React.FC<{ tool: Tool; onClose: () => void; onConfirm: (tool: Tool, siteAddress: string) => void }> = ({ tool, onClose, onConfirm }) => {
  ...
  const handleAddressChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (value.trim().length >= 3) {
      ...
      // Fast narrowing debounce (250ms)
      searchTimeoutRef.current = window.setTimeout(async () => {
        const results = await searchAddresses(value);
        // Persist old suggestions while loading new ones for smoother narrowing
        ...
        <div className="space-y-5">
          <div className="space-y-1 relative">
            ...
            {(suggestions.length > 0 || (hasAttemptedSearch && !isSearching)) && (
              <div className="absolute top-full left-0 right-0 mt-2 bg-white rounded-2xl shadow-2xl border border-slate-100 overflow-hidden z-[720] max-h-[250px] overflow-y-auto hide-scrollbar">
                {suggestions.length > 0 ? (
                  suggestions.map((addr, idx) => (
                    <button 
                      key={idx} 
                      onPointerDown={(e) => handleSelectSuggestion(e, addr)} 
                      className="w-full text-left px-5 py-4 text-[11px] font-bold text-slate-600 hover:bg-slate-50 flex items-center gap-3 border-b border-slate-50 last:border-0 active:bg-slate-100 transition-colors"
                    >
                  ...
                  <div className="px-5 py-10 text-center flex flex-col items-center gap-3">
                    <div className="bg-slate-50 p-4 rounded-full"><MapPin size={20} className="text-slate-200" /></div>
                    <p className="text-[10px] font-black text-slate-300 uppercase tracking-[0.2em]">No results for this search</p>
                  </div>
                )}
              </div>
            )}
